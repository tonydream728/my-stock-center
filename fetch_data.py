...
try:
    ...
    prof_db = {}
    if prof_files:
        df_prof = pd.read_csv(prof_files[0], encoding='cp950')
    ...
    for f in [f for f in glob.glob("t05st09_new_*.csv") if "t51sb01" not in f]:
        # 開啟檔案時也指定 encoding='cp950'
        df = pd.read_csv(f, skiprows=next(i for i, l in enumerate(open(f, encoding='cp950')) if "公司代號名稱" in l), on_bad_lines='skip', encoding='cp950')
    ...
    for item in data:
        ...
        if len(code) == 4:
            output.append({
                ...
                "yield": item.get('DividendYield', '0'), "pe": item.get('PEratio', 'N/A'),
